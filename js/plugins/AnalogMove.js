 ...
 * @version 1.26 2015/11/20 フォロワーの位置反映、イベント中の歩行アニメ、通行判定を暫定修正
...
// (function() {
    ...
    Game_AnalogMove.prototype.updateNextRealXY = function(thisCharacter) {
        ...
        splitDistances.forEach(function(splitDistance) {
            ...
            var collideWallXY = this.collideWallXY(splitDistance);
    ...
    // 衝突する壁の座標
    Game_AnalogMove.prototype.collideWallXY = function(distancePerFrame) {
        ...
        if (this._crntRealX >= this._nextRealX) {
            // 左の壁
            var x2 = (Math.floor(nextCenterX - 0.5) + $gameMap.width())  % $gameMap.width();
            var tile4 = $gameMap.collideMap.tiles[x2][y];
            ...
            // 右の壁
            var x2 = (Math.floor(nextCenterX + 0.5) + $gameMap.width())  % $gameMap.width();
            var tile6 = $gameMap.collideMap.tiles[x2][y];
        ...
        if (this._crntRealY >= this._nextRealY) {
            // 上の壁
            var y2 = (Math.floor(nextCenterY - 0.5) + $gameMap.height()) % $gameMap.height();
            var tile8 = $gameMap.collideMap.tiles[x][y2];
            ...
            // 下の壁
            var y2 = (Math.floor(nextCenterY + 0.5) + $gameMap.height()) % $gameMap.height();
            var tile2 = $gameMap.collideMap.tiles[x][y2];
    ...
    // システムの初期化
    var _rn_AnalogMove_Game_System_initialize = Game_System.prototype.initialize;
    Game_System.prototype.initialize = function() {
        _rn_AnalogMove_Game_System_initialize.call(this);
        this._analogMoveValid = PluginManager.parameters('AnalogMove')['Valid'] === 'ON';
    };
    ...
    // キャラクターベースクラスの方向設定
    var _rn_AnalogMove_Game_CharacterBase_setDirection = Game_CharacterBase.prototype.setDirection;
    Game_CharacterBase.prototype.setDirection = function(d) {
        _rn_AnalogMove_Game_CharacterBase_setDirection.call(this, d);
        if (this._direction !== this._analogMove.dir4() && !this.isDirectionFixed()) {
